var Dot = new function () {
    ...
    function gameOver() {
        ...
        scoreText = 'Sdot: <span>' + Math.round(score) + '</span>';
        scoreText += ' Time: <span>' + Math.round(((new Date()
            .getTime() - time) / 1000) * 100) / 100 + 's</span>';
    ...
    function animate() {
        ...
        if (playing) {
            ...
            var loopedNodes = player.dotNodes.concat();
            loopedNodes.push(player.dotNodes[0]);
        ...
        if (enemyCount < 1 * difficulty && new Date()
            .getTime() - lastspawn > 100) {
    ...
    function giveLife(organism) {
        ...
        if (playing) DotAudio.playSynth(organism.position.x / world.width);
...
Player.prototype.updateDot = function () {
    var i, j, n;
    ...
    if (this.dotNodes.length == 0) {
        ...
        for (i = 0; i < this.dotQuality; i++) {
            ...
            this.dotNodes.push(n);
        }
    }

    for (i = 0; i < this.dotQuality; i++) {

        var n = this.dotNodes[i];

        var angle = (i / this.dotQuality) * Math.PI * 2;
