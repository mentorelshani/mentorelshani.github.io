var DotAudio = new function() {
    ...
    var shieldNoiseOsc = T("fnoise", {
        freq: 50,
        mul: 0.1
    });
    var shieldOsc = T("pulse", {
        freq: 50,
        mul: 0.1
    });
    var shieldEnv = T("perc", {
        a: 10,
        r: 100
    }, shieldNoiseOsc, shieldOsc).play();
    ...
    this.playSynth = function(objPos) {
        ...
        if (Math.random() < 0.7) {
            var playOsc = T("sin", {
                freq: midicps.at(60 + note + octave),
                mul: 0.2
            });
            var playEnv = T("perc", {
                r: 3000
            ...
            var tremParam = T("param", {
                value: 1
            }).to(Math.random() * 5 + 5, 3000);
            var tremOsc = T("sin", {
                freq: tremParam,
                mul: .6
            });
            var trem = T("*", eq, tremOsc);
            var panParam = T("param", {
                value: objPos
            }).expTo(0.5, 3000);
            var pan = T("pan", {
                pos: panParam
